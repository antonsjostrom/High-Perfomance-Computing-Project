...
int place_piece(unsigned short int cross, unsigned short int circle, int threads) {
	...
	#pragma omp parallel for num_threads(threads) firstprivate(cross,circle) schedule(dynamic,1)
	for (i = 0; i < 16; i++) {
		if (possible_move(cross, circle, i)) {
			circle |= 1 << i;
...
int main(int argc, char const* argv[]) {
	...
	if (argc != 2) {
		printf("expected 1 input argument, number of threads \n");
		return 0;
	}
	...
	int index = place_piece(boardToInt(cross_board), boardToInt(circle_board), threads);
