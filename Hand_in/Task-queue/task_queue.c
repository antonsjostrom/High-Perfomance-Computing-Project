...
int minimax_tasked(unsigned short int cross, unsigned short int circle, char player, int alpha, int beta, int depth, int startindex, int* recursive_array) {
	//Check winner or draw:
	if (win(cross)) {
		return -1;
	}
	...
	if (depth > 0) {
		if (player == 'o') {
			...
			for (i = 0; i < 16; i++) {
				if (possible_move(cross, circle, i)) {
					#pragma omp task firstprivate(cross, circle, i, alpha, beta, depth)
					{
						circle |= 1 << i; 
						recursive_array[startindex + i] = minimax_tasked(cross, circle, 'x', alpha, beta, depth-1, (i+1)*startindex/17, recursive_array);
					}   //Do not need to reset, since own private copy
				}
			}
			#pragma omp taskwait
			int max = -1;
		...
		for (i = 0; i < 16; i++) {
			if (possible_move(cross, circle, i)) {
				#pragma omp task firstprivate(cross, circle, i, alpha, beta, depth, startindex)
				{
					cross |= 1 << i;   
					recursive_array[startindex + i] = minimax_tasked(cross, circle, 'o', alpha, beta, depth-1, (i+1)*startindex/17, recursive_array);
				}
			}
		}
		#pragma omp taskwait
	...
	if (player == 'o') {
		...
		for (i = 0; i < 16; i++) {
			if (possible_move(cross, circle, i)) {
				circle |= 1 << i;   
				recursive_array[startindex + i] = minimax(cross, circle, 'x', alpha, beta);
				circle &= ~(1 << i);
				}
			}
	...
	for (i = 0; i < 16; i++) {
		if (possible_move(cross, circle, i)) {
			cross |= 1 << i;   
			recursive_array[startindex + i] = minimax(cross, circle, 'o', alpha, beta);
			cross &= ~(1 << i);
		}
	}
...
int place_piece(unsigned short int cross, unsigned short int circle, int threads, int depth) {
			...
			for (i = 0; i < 16; i++) {
				if (possible_move(cross, circle, i)) {
					#pragma omp task firstprivate(cross, circle, i, alpha, beta, depth)
					{
						circle |= 1 << i;   
						if (depth > 1) {
							eval_array[i] = minimax_tasked(cross, circle, 'x', alpha, beta, depth-1, (i+1)*rec_size/17, recursive_array);
						}
						else {
							eval_array[i] = minimax(cross, circle, 'x', alpha, beta);
						}
					}
				}
			}
	...
	//no need to use taskwait since implicit barrier
